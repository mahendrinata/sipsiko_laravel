...
abstract class Crypt_Hash_TestCase extends PhpseclibTestCase {
  ...
  protected function assertHashesTo(Crypt_Hash $hash, $message, $expected) {
    $this->assertEquals(
      strtolower($expected), bin2hex($hash->hash($message)), sprintf("Failed asserting that '%s' hashes to '%s'.", $message, $expected)
    );
  }
  ...
  protected function assertHMACsTo(Crypt_Hash $hash, $key, $message, $expected) {
    ...
    $this->assertEquals(
      strtolower($expected), bin2hex($hash->hash($message)), sprintf("Failed asserting that '%s' HMACs to '%s' with key '%s'.", $message, $expected, $key)
    );
  }
