...
class KeySortTest extends PredisCommandTestCase {
  ...
  public function testSortWithAscOrDescModifier() {
    ...
    $this->assertEquals(
      array(100, 30, 10, 3, 2, 1), $redis->sort('list:unordered', array(
        'sort' => 'desc',
      ))
    );

    $this->assertEquals(
      array(1, 2, 3, 10, 30, 100), $redis->sort('list:unordered', array(
        'sort' => 'asc',
      ))
    );
  }
  ...
  public function testSortWithLimitModifier() {
    ...
    $this->assertEquals(
      array(1, 2, 3), $redis->sort('list:unordered', array(
        'limit' => array(0, 3),
      ))
    );

    $this->assertEquals(
      array(10, 30), $redis->sort('list:unordered', array(
        'limit' => array(3, 2)
      ))
    );
  }
  ...
  public function testSortWithAlphaModifier() {
    ...
    $this->assertEquals(
      array(1, 10, 100, 2, 3, 30), $redis->sort('list:unordered', array(
        'alpha' => true
      ))
    );
  }
  ...
  public function testSortWithStoreModifier() {
    ...
    $this->assertEquals(
      count($unordered), $redis->sort('list:unordered', array(
        'store' => 'list:ordered'
      ))
    );
  ...
  public function testSortWithCombinedModifiers() {
    ...
    $this->assertEquals(
      array(30, 10, 3, 2), $redis->sort('list:unordered', array(
        'alpha' => false,
        'sort' => 'desc',
        'limit' => array(1, 4)
      ))
