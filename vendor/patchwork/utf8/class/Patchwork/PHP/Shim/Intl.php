...
class Intl {
  ...
  static function grapheme_extract($s, $size, $type = GRAPHEME_EXTR_COUNT, $start = 0, &$next = 0) {
    ...
    if ('' === $s || 0 > $size || 0 > $start || 0 > $type || 2 < $type)
      return false;
    if (0 === $size)
      return '';
  ...
  protected static function grapheme_position($s, $needle, $offset, $mode) {
    ...
    switch ($mode) {
      case 0: $needle = iconv_strpos($s, $needle, 0, 'UTF-8');
        break;
      case 1: $needle = mb_stripos($s, $needle, 0, 'UTF-8');
        break;
      case 2: $needle = iconv_strrpos($s, $needle, 'UTF-8');
        break;
      default: $needle = mb_strripos($s, $needle, 0, 'UTF-8');
        break;
    }
