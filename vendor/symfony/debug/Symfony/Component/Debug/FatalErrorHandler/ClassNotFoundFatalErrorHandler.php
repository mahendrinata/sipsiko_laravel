  ...
  public function handleError(array $error, FatalErrorException $exception) {
    ...
    foreach (array('class', 'interface', 'trait') as $typeName) {
      ...
      if (false !== $namespaceSeparatorIndex = strrpos($fullyQualifiedClassName, '\\')) {
        ...
        $message = sprintf(
          'Attempted to load %s "%s" from namespace "%s" in %s line %d. Do you need to "use" it from another namespace?', $typeName, $className, $namespacePrefix, $error['file'], $error['line']
        );
      } else {
        $className = $fullyQualifiedClassName;
        $message = sprintf(
          'Attempted to load %s "%s" from the global namespace in %s line %d. Did you forget a use statement for this %s?', $typeName, $className, $error['file'], $error['line'], $typeName
        );
      }
