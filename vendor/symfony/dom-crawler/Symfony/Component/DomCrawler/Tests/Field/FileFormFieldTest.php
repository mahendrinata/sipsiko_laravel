...
class FileFormFieldTest extends FormFieldTestCase {
  ...
  public function testSetValue($method) {
    ...
    $this->assertEquals(
      $origInfo['extension'], $tmpInfo['extension'], "->$method() keeps the same file extension in the tmp_name copy"
    );
    ...
    $this->assertArrayNotHasKey(
      'extension', pathinfo($value['tmp_name']), "->$method() does not add a file extension in the tmp_name copy"
    );
  }
