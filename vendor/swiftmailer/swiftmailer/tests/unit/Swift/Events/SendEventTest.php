...
class Swift_Events_SendEventTest extends Swift_Tests_SwiftUnitTestCase {
  ...
  public function testMessageCanBeFetchedViaGetter() {
    ...
    $ref = $evt->getMessage();
    $this->assertReference($message, $ref, '%s: Message should be returned from getMessage()'
    );
  }
  ...
  public function testTransportCanBeFetchViaGetter() {
    ...
    $this->assertReference($transport, $ref, '%s: Transport should be returned from getTransport()'
    );
  }
  ...
  public function testTransportCanBeFetchViaGetSource() {
    ...
    $ref = $evt->getSource();
    $this->assertReference($transport, $ref, '%s: Transport should be returned from getSource()'
    );
  }
  ...
  public function testFailedRecipientsCanBeSetAndGet() {
    ...
    $this->assertEqual(array('foo@bar', 'zip@button'), $evt->getFailedRecipients(), '%s: FailedRecipients should be returned from getter'
    );
  }
