...
abstract class Swift_Transport_AbstractSmtpTest extends Swift_Tests_SwiftUnitTestCase {
  ...
  public function testSendReturnsNumberOfSuccessfulRecipients() {
    ...
    $this->_finishBuffer($buf);
    $smtp->start();
    $this->assertEqual(2, $smtp->send($message), '%s: 1 of 3 recipients failed so 2 should be returned'
    );
  }
  ...
  public function testRsetIsSentIfNoSuccessfulRecipients() {
    ...
    $this->_finishBuffer($buf);
    $smtp->start();
    $this->assertEqual(0, $smtp->send($message), '%s: 1 of 1 recipients failed so 0 should be returned'
    );
  }
  ...
  public function testFailedRecipientsCanBeCollectedByReference() {
    ...
    $this->assertEqual(array('zip@button', 'test@domain'), $failures, '%s: Failures should be caught in an array'
    );
  }
