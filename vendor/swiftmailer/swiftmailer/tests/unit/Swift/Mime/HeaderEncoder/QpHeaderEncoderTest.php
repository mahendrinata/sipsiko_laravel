...
class Swift_Mime_HeaderEncoder_QpHeaderEncoderTest extends Swift_Tests_SwiftUnitTestCase {
  ...
  public function testSpaceAndTabNeverAppear() {
    ...
    $this->assertNoPattern('~[ \t]~', $encoder->encodeString("a \t b"), '%s: encoded-words in headers cannot contain LWSP as per RFC 2047.'
    );
  }
  ...
  public function testSpaceIsRepresentedByUnderscore() {
    ...
    $encoder = $this->_createEncoder($charStream);
    $this->assertEqual('a_b', $encoder->encodeString('a b'), '%s: Spaces can be represented by more readable underscores as per RFC 2047.'
    );
  }
  ...
  public function testEqualsAndQuestionAndUnderscoreAreEncoded() {
    ...
    $encoder = $this->_createEncoder($charStream);
    $this->assertEqual('=3D=3F=5F', $encoder->encodeString('=?_'), '%s: Chars =, ? and _ (underscore) may not appear as per RFC 2047.'
    );
  }
  ...
  public function testParensAndQuotesAreEncoded() {
    ...
    $encoder = $this->_createEncoder($charStream);
    $this->assertEqual('=28=22=29', $encoder->encodeString('(")'), '%s: Chars (, " (DQUOTE) and ) may not appear as per RFC 2047.'
    );
  }
  ...
  public function testOnlyCharactersAllowedInPhrasesAreUsed() {
    ...
    foreach (range(0x00, 0xFF) as $byte) {
      ...
      if (in_array($byte, $allowedBytes)) {
        $this->assertEqual($char, $encodedChar, '%s: Character ' . $char . ' should not be encoded.'
        );
      } elseif (0x20 == $byte) { //Special case
        $this->assertEqual('_', $encodedChar, '%s: Space character should be replaced.'
        );
      } else {
        $this->assertEqual(sprintf('=%02X', $byte), $encodedChar, '%s: Byte ' . $byte . ' should be encoded.'
        );
      }
    }
  }
  ...
  private function _createCharacterStream($stub = false) {
    return $stub ? $this->_stub('Swift_CharacterStream') : $this->_mock('Swift_CharacterStream')
    ;
  }
