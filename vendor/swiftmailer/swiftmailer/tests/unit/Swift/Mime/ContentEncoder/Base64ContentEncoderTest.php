...
class Swift_Mime_ContentEncoder_Base64ContentEncoderTest extends Swift_Tests_SwiftUnitTestCase {
  ...
  public function testPadLength() {
    ...
    for ($i = 0; $i < 30; ++$i) {
      ...
      $this->_encoder->encodeByteStream($os, $is);
      $this->assertPattern('~^[a-zA-Z0-9/\+]{2}==$~', $collection->content, '%s: A single byte should have 2 bytes of padding'
      );
    }
    ...
    for ($i = 0; $i < 30; ++$i) {
      ...
      $this->_encoder->encodeByteStream($os, $is);
      $this->assertPattern('~^[a-zA-Z0-9/\+]{3}=$~', $collection->content, '%s: Two bytes should have 1 byte of padding'
      );
    }
    ...
    for ($i = 0; $i < 30; ++$i) {
      ...
      $this->_encoder->encodeByteStream($os, $is);
      $this->assertPattern('~^[a-zA-Z0-9/\+]{4}$~', $collection->content, '%s: Three bytes should have no padding'
      );
    }
  }
  ...
  private function _createOutputByteStream($stub = false) {
    return $stub ? $this->_stub('Swift_OutputByteStream') : $this->_mock('Swift_OutputByteStream')
    ;
  }

  private function _createInputByteStream($stub = false) {
    return $stub ? $this->_stub('Swift_InputByteStream') : $this->_mock('Swift_InputByteStream')
    ;
  }
