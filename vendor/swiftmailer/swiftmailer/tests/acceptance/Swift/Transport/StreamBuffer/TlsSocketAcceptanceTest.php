...
class Swift_Transport_StreamBuffer_TlsSocketAcceptanceTest extends Swift_Transport_StreamBuffer_AbstractStreamBufferAcceptanceTest {
  ...
  public function skip() {
    ...
    $this->skipIf(!in_array('tls', $streams), 'TLS is not configured for your system.  It is not possible to run this test'
    );
    $this->skipIf(!SWIFT_TLS_HOST, 'Cannot run test without a TLS enabled SMTP host to connect to (define ' .
