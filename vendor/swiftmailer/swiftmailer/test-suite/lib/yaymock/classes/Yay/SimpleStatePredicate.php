...
class Yay_SimpleStatePredicate implements Yay_StatePredicate {
  ...
  public function isActive() {
    return (($this->_is && $this->_stateMachine->getCurrentState() == $this->_stateName) || (!$this->_is && $this->_stateMachine->getCurrentState() != $this->_stateName));
  }
  ...
  public function describeTo(Yay_Description $description) {
    ...
    $description->appendText(sprintf(
        ' %s %s;', ($this->_is ? 'is' : 'is not'), $this->_stateName
    ));
  }
