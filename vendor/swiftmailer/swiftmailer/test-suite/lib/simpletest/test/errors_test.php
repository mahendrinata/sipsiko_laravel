...
class TestOfPHP52RecoverableErrors extends UnitTestCase {
  ...
  function skip() {
    $this->skipIf(
      version_compare(phpversion(), '5.2', '<'), 'E_RECOVERABLE_ERROR not tested for PHP below 5.2');
  }
...
class TestOfErrorsExcludingPHP52AndAbove extends UnitTestCase {
  ...
  function skip() {
    $this->skipIf(
      version_compare(phpversion(), '5.2', '>='), 'E_USER_ERROR not tested for PHP 5.2 and above');
  }
