...
Mock::generatePartial(
  'SimpleBrowser', 'MockParseSimpleBrowser', array('createUserAgent', 'parse'));
Mock::generatePartial(
  'SimpleBrowser', 'MockUserAgentSimpleBrowser', array('createUserAgent'));
...
class TestOfHistory extends UnitTestCase {
  ...
  function testCurrentTargetAccessors() {
    $history = new SimpleBrowserHistory();
    $history->recordEntry(
      new SimpleUrl('http://www.here.com/'), new SimpleGetEncoding());
  ...
  function testSecondEntryAccessors() {
    $history = new SimpleBrowserHistory();
    $history->recordEntry(
      new SimpleUrl('http://www.first.com/'), new SimpleGetEncoding());
    $history->recordEntry(
      new SimpleUrl('http://www.second.com/'), new SimplePostEncoding(array('a' => 1)));
    $this->assertIdentical($history->getUrl(), new SimpleUrl('http://www.second.com/'));
    $this->assertIdentical(
      $history->getParameters(), new SimplePostEncoding(array('a' => 1)));
  }
  ...
  function testNewEntryReplacesNextOne() {
    ...
    $history->back();
    $history->recordEntry(
      new SimpleUrl('http://www.third.com/'), new SimpleGetEncoding());
  ...
  function testNewEntryDropsFutureEntries() {
    ...
    $history->recordEntry(
      new SimpleUrl('http://www.first.com/'), new SimpleGetEncoding());
    $history->recordEntry(
      new SimpleUrl('http://www.second.com/'), new SimpleGetEncoding());
    $history->recordEntry(
      new SimpleUrl('http://www.third.com/'), new SimpleGetEncoding());
    $history->back();
    $history->back();
    $history->recordEntry(
      new SimpleUrl('http://www.fourth.com/'), new SimpleGetEncoding());
...
class TestOfBrowserNavigation extends UnitTestCase {
  ...
  function testClickLinkRequestsPage() {
    ...
    $agent->expectAt(
      0, 'fetchResponse', array(new SimpleUrl('http://this.com/page.html'), new SimpleGetEncoding()));
    $agent->expectAt(
      1, 'fetchResponse', array(new SimpleUrl('http://this.com/new.html'), new SimpleGetEncoding()));
  ...
  function testClickLinkWithUnknownFrameStillRequestsWholePage() {
    ...
    $agent->expectAt(
      1, 'fetchResponse', array($target, new SimpleGetEncoding()));
    $agent->expectCallCount('fetchResponse', 2);
  ...
  function testClickIndexedLink() {
    ...
    $agent->returns('fetchResponse', new MockSimpleHttpResponse());
    $agent->expectAt(
      1, 'fetchResponse', array(new SimpleUrl('1.html'), new SimpleGetEncoding()));
    $agent->expectCallCount('fetchResponse', 2);

    $page = new MockSimplePage();
    $page->setReturnValue(
      'getUrlsByLabel', array(new SimpleUrl('0.html'), new SimpleUrl('1.html')));
    $page->setReturnValue('getRaw', 'A page');
...
class TestOfBrowserFrames extends UnitTestCase {
  ...
  function testSimplePageHasNoFrames() {
    ...
    $this->assertEqual(
      $browser->get('http://site.with.no.frames/'), 'A non-framed page');
  ...
  function testFramesetWithSingleFrame() {
    ...
    $this->assertIdentical(
      $browser->getFrames(), array('a' => 'http://site.with.one.frame/frame.html'));
  }
  ...
  function testFramesetWithSingleUnnamedFrame() {
    ...
    $this->assertEqual(
      $browser->get('http://site.with.one.frame/'), 'One frame');
    $this->assertIdentical(
      $browser->getFrames(), array(1 => 'http://site.with.one.frame/frame.html'));
  }
  ...
  function testFramesetWithMultipleFrames() {
    ...
    $this->assertEqual(
      $browser->get('http://site.with.frames/'), 'A frameB frameC frame');
    $this->assertIdentical($browser->getFrames(), array(
  ...
  function testFramesetWithSomeNamedFrames() {
    ...
    $this->assertEqual(
      $browser->get('http://site.with.frames/'), 'A frameB frameC frameD frame');
    $this->assertIdentical($browser->getFrames(), array(
      'a' => 'http://site.with.frames/frame_a.html',
  ...
  function testNestedFrameset() {
    ...
    $this->assertEqual(
      $browser->get('http://site.with.nested.frame/'), 'The page');
    $this->assertIdentical($browser->getFrames(), array(
      'inner' => array(
