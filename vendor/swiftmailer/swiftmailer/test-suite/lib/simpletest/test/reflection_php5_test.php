...
class TestOfReflection extends UnitTestCase {
  ...
  function testInterfaceHasOnlyItselfToImplement() {
    $reflection = new SimpleReflection('AnyOldInterface');
    $this->assertEqual(
      $reflection->getInterfaces(), array('AnyOldInterface'));
  }
  ...
  function testCanProperlyGenerateStaticMethodSignatures() {
    ...
    $this->assertEqual(
      'static function aStaticWithParameters($arg1, $arg2)', $reflection->getSignature('aStaticWithParameters')
    );
  }
...
class TestOfAbstractsWithAbstractMethods extends UnitTestCase {
  ...
  function testCanProperlyGenerateAbstractMethods() {
    ...
    $this->assertEqual(
      'function anAbstract()', $reflection->getSignature('anAbstract')
    );
    $this->assertEqual(
      'function anAbstractWithParameter($foo)', $reflection->getSignature('anAbstractWithParameter')
    );
    $this->assertEqual(
      'function anAbstractWithMultipleParameters($foo, $bar)', $reflection->getSignature('anAbstractWithMultipleParameters')
    );
  }
