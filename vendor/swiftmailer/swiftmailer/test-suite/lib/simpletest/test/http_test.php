...
class TestOfProxyRoute extends UnitTestCase {
  ...
  function testDefaultGet() {
    ...
    $route->setReturnReference('createSocket', $socket);
    $route->__construct(
      new SimpleUrl('http://a.valid.host/here.html'), new SimpleUrl('http://my-proxy'));
    $route->createConnection('GET', 15);
  }
  ...
  function testGetWithPort() {
    ...
    $route->setReturnReference('createSocket', $socket);
    $route->__construct(
      new SimpleUrl('http://a.valid.host:81/here.html'), new SimpleUrl('http://my-proxy:8081'));
    $route->createConnection('GET', 15);
  }
  ...
  function testGetWithParameters() {
    ...
    $route->setReturnReference('createSocket', $socket);
    $route->__construct(
      new SimpleUrl('http://a.valid.host/here.html?a=1&b=2'), new SimpleUrl('http://my-proxy'));
    $route->createConnection('GET', 15);
  }
  ...
  function testGetWithAuthentication() {
    ...
    $route->setReturnReference('createSocket', $socket);
    $route->__construct(
      new SimpleUrl('http://a.valid.host/here.html'), new SimpleUrl('http://my-proxy'), 'Me', 'Secret');
    $route->createConnection('GET', 15);
  }
...
class TestOfHttpPostRequest extends UnitTestCase {
  ...
  function testContentHeadersCalculated() {
    ...
    $request = new SimpleHttpRequest(
      $route, new SimplePostEncoding(array('a' => 'A')));
    $this->assertIsA($request->fetch(15), 'SimpleHttpResponse');
  }
...
class TestOfHttpResponse extends UnitTestCase {
  ...
  function testParseOfResponseHeadersWhenChunked() {
    ...
    $this->assertEqual(
      $response->getContent(), "this is a test file\nwith two lines in it\n");
