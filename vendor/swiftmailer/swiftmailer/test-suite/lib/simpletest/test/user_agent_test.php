...
class TestOfFetchingUrlParameters extends UnitTestCase {
  ...
  function testGetRequestWithoutIncidentGivesNoErrors() {
    ...
    $response = $agent->fetchResponse(
      new SimpleUrl('http://test:secret@this.com/page.html'), new SimpleGetEncoding(array('a' => 'A', 'b' => 'B')));
...
class TestOfAdditionalHeaders extends UnitTestCase {
  ...
  function testAdditionalHeaderAddedToRequest() {
    ...
    $request->expectOnce(
      'addHeaderLine', array('User-Agent: SimpleTest'));
...
class TestOfBrowserCookies extends UnitTestCase {
  ...
  function testCookieJarIsSentToRequest() {
    ...
    $agent->setCookie('a', 'A');
    $agent->fetchResponse(
      new SimpleUrl('http://this.com/this/path/page.html'), new SimpleGetEncoding());
  }
  ...
  function testClearCookieBySettingExpiry() {
    ...
    $this->assertIdentical(
      $agent->getCookieValue("this.com", "this/path/", "a"), "b");
    $agent->restart("Wed, 25-Dec-02 04:24:20 GMT");
    $this->assertIdentical(
      $agent->getCookieValue("this.com", "this/path/", "a"), false);
  }
  ...
  function testAgeingAndClearing() {
    ...
    $agent->restart("Wed, 25-Dec-02 04:24:20 GMT");
    $this->assertIdentical(
      $agent->getCookieValue("this.com", "this/path/", "a"), "A");
    $agent->ageCookies(2);
    $agent->restart("Wed, 25-Dec-02 04:24:20 GMT");
    $this->assertIdentical(
      $agent->getCookieValue("this.com", "this/path/", "a"), false);
  ...
  function testReadingIncomingAndSettingNewCookies() {
    ...
    $this->assertEqual(
      $agent->getBaseCookieValue("a", new SimpleUrl('http://this.com/this/path/page.html')), "AAA");
    $this->assertEqual(
      $agent->getBaseCookieValue("b", new SimpleUrl('http://this.com/this/path/page.html')), "BBB");
  }
...
class TestOfHttpRedirects extends UnitTestCase {
  ...
  function testDisabledRedirects() {
    $agent = new MockRequestUserAgent();
    $agent->returns(
      'createHttpRequest', $this->createRedirect('stuff', 'there.html'));
  ...
  function testSingleRedirect() {
    $agent = new MockRequestUserAgent();
    $agent->returnsAt(
      0, 'createHttpRequest', $this->createRedirect('first', 'two.html'));
    $agent->returnsAt(
      1, 'createHttpRequest', $this->createRedirect('second', 'three.html'));
  ...
  function testDoubleRedirect() {
    ...
    $agent->returnsAt(
      1, 'createHttpRequest', $this->createRedirect('second', 'three.html'));
    $agent->returnsAt(
      2, 'createHttpRequest', $this->createRedirect('third', 'four.html'));
  ...
  function testSuccessAfterRedirect() {
    ...
    $agent->returnsAt(
      0, 'createHttpRequest', $this->createRedirect('first', 'two.html'));
    $agent->returnsAt(
      1, 'createHttpRequest', $this->createRedirect('second', false));
    $agent->returnsAt(
      2, 'createHttpRequest', $this->createRedirect('third', 'four.html'));
    $agent->expectCallCount('createHttpRequest', 2);
    $agent->__construct();
...
class TestOfBadHosts extends UnitTestCase {
  ...
  function testUntestedHost() {
    ...
    $agent->__construct();
    $response = $agent->fetchResponse(
      new SimpleUrl('http://this.host/this/path/page.html'), new SimpleGetEncoding());
...
class TestOfAuthorisation extends UnitTestCase {
  ...
  function testAuthenticateHeaderAdded() {
    ...
    $request->expectOnce(
      'addHeaderLine', array('Authorization: Basic ' . base64_encode('test:secret')));

    $agent = new MockRequestUserAgent();
    $agent->returns('createHttpRequest', $request);
    $agent->__construct();
    $response = $agent->fetchResponse(
      new SimpleUrl('http://test:secret@this.host'), new SimpleGetEncoding());
  }
