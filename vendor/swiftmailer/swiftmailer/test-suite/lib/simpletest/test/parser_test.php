...
class TestOfHtmlSaxParser extends UnitTestCase {
  ...
  function testTagWithUnquotedAttributes() {
    $listener = &$this->createListener();
    $listener->expectOnce(
      'startElement', array('input', array('name' => 'a.b.c', 'value' => 'd')));
    $parser = new SimpleHtmlSaxParser($listener);
  ...
  function testTagWithEmptyAttributes() {
    $listener = &$this->createListener();
    $listener->expectOnce(
      'startElement', array('option', array('value' => '', 'selected' => '')));
    $listener->expectOnce('addContent', array('label'));
  ...
  function testComplexTagWithLotsOfCaseVariations() {
    $listener = &$this->createListener();
    $listener->expectOnce(
      'startElement', array('a', array('href' => 'here.html', 'style' => "'cool'")));
    $listener->expectOnce('addContent', array('label'));
    $listener->expectOnce('endElement', array('a'));
    $parser = new SimpleHtmlSaxParser($listener);
  ...
  function testXhtmlSelfClosingTag() {
    $listener = &$this->createListener();
    $listener->expectOnce(
      'startElement', array('input', array('type' => 'submit', 'name' => 'N', 'value' => 'V')));
    $parser = new SimpleHtmlSaxParser($listener);
...
class TestOfTextExtraction extends UnitTestCase {
  ...
  function testImageSuppressionWhileKeepingParagraphsAndAltText() {
    $this->assertEqual(
      SimpleHtmlSaxParser::normalise('<img src="foo.png" /><p>some text</p><img src="bar.png" alt="bar" />'), 'some text bar');
  }
  ...
  function testTagSuppression() {
    $this->assertEqual(
      SimpleHtmlSaxParser::normalise('<b>Hello</b>'), 'Hello');
  }

  function testAdjoiningTagSuppression() {
    $this->assertEqual(
      SimpleHtmlSaxParser::normalise('<b>Hello</b><em>Goodbye</em>'), 'HelloGoodbye');
  }

  function testExtractImageAltTextWithDifferentQuotes() {
    $this->assertEqual(
      SimpleHtmlSaxParser::normalise('<img alt="One"><img alt=\'Two\'><img alt=Three>'), 'One Two Three');
  }
