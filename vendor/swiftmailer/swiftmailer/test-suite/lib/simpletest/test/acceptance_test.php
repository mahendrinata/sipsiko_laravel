...
class TestOfLivePageLinkingWithMinimalLinks extends SimpleTestAcceptanceTest {
  ...
  function testClickToSingleDotGoesToCurrentDirectory() {
    ...
    $this->assertTitle(
      'Simple test front controller', '%s -> index.php needs to be set as a default web server home page');
  }
...
class TestOfLiveCookies extends SimpleTestAcceptanceTest {
  ...
  function testNoClockOverDriftBy100Seconds() {
    ...
    $this->assertNoCookie(
      'short_cookie', '%s -> Please check your computer clock setting if you are not using NTP');
  }
  ...
  function testNoClockUnderDriftBy100Seconds() {
    ...
    $this->restart(time() + 0);
    $this->assertCookie(
      'short_cookie', 'B', '%s -> Please check your computer clock setting if you are not using NTP');
  }
...
class TestOfFileUploads extends SimpleTestAcceptanceTest {
  ...
  function testSingleFileUpload() {
    ...
    $this->assertTrue($this->setField('Content:', dirname(__FILE__) . '/support/upload_sample.txt'));
  ...
  function testMultipleFileUpload() {
    $this->get($this->samples() . 'upload_form.html');
    $this->assertTrue($this->setField('Content:', dirname(__FILE__) . '/support/upload_sample.txt'));
    $this->assertTrue($this->setField('Supplemental:', dirname(__FILE__) . '/support/supplementary_upload_sample.txt'));
    $this->assertField('Supplemental:', dirname(__FILE__) . '/support/supplementary_upload_sample.txt');
    $this->click('Go!');
    $this->assertText('Sample for testing file upload');
  ...
  function testBinaryFileUpload() {
    $this->get($this->samples() . 'upload_form.html');
    $this->assertTrue($this->setField('Content:', dirname(__FILE__) . '/support/latin1_sample'));
    $this->click('Go!');
    $this->assertText(
...
class TestOfLiveHistoryNavigation extends SimpleTestAcceptanceTest {
  ...
  function testGetRetryResubmitsExtraData() {
    $this->assertTrue($this->get(
        $this->samples() . 'network_confirm.php', array('a' => 'aaa')));
    $this->assertPattern('/Request method.*?<dd>GET<\/dd>/');
    $this->assertText('a=[aaa]');
    $this->retry();
    $this->assertPattern('/Request method.*?<dd>GET<\/dd>/');
