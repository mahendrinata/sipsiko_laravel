...
Mock::generatePartial(
  'SimplePageBuilder', 'PartialSimplePageBuilder', array('createPage', 'createParser'));
...
class TestOfPageBuilder extends UnitTestCase {
  ...
  function testLinkWithId() {
    ...
    $builder->parse(new MockSimpleHttpResponse());
    $this->assertTrue($builder->startElement(
        "a", array("href" => "http://somewhere", "id" => "44")));
    $this->assertTrue($builder->addContent("Label"));
    $this->assertTrue($builder->endElement("a"));
  }
...
class TestOfHtmlPage extends UnitTestCase {
  ...
  function testAddAbsoluteLink() {
    ...
    $page->AcceptTag($link);
    $this->assertEqual(
      $page->getUrlsByLabel('Label'), array(new SimpleUrl('http://somewhere.com')));
  }
  ...
  function testAddStrictRelativeLink() {
    ...
    $page->AcceptTag($link);
    $this->assertEqual(
      $page->getUrlsByLabel('Label'), array(new SimpleUrl('http://host/somewhere.php')));
  }
  ...
  function testAddRelativeLinkWithBaseTag() {
    ...
    $page->AcceptTag($base);
    $this->assertEqual(
      $page->getUrlsByLabel('Label'), array(new SimpleUrl('www.lastcraft.com/stuff/somewhere.php')));
  }
  ...
  function testAddAbsoluteLinkWithBaseTag() {
    ...
    $page->AcceptTag($base);
    $this->assertEqual(
      $page->getUrlsByLabel('Label'), array(new SimpleUrl('http://here.com/somewhere.php')));
  }
  ...
  function testLinkIds() {
    ...
    $this->assertEqual(
      $page->getUrlById(33), new SimpleUrl('http://host/somewhere.php'));
  }
  ...
  function testFindLinkWithNormalisation() {
    ...
    $this->assertEqual(
      $page->getUrlsByLabel('Long & thin'), array(new SimpleUrl('http://host/somewhere.php')));
  }
  ...
  function testFindLinkWithImage() {
    ...
    $this->assertEqual(
      $page->getUrlsByLabel('<A picture>'), array(new SimpleUrl('http://host/somewhere.php')));
  }
  ...
  function testRelativeFramesRespectBaseTag() {
    ...
    $this->assertIdentical(
      $page->getFrameset(), array(1 => new SimpleUrl('https://there.com/stuff/1.html')));
  }
...
class TestOfFormsCreatedFromEventStream extends UnitTestCase {
  ...
  function testFormCanBeSubmitted() {
    ...
    $this->assertEqual(
      $form->submitButton(new SimpleByLabel('Submit')), new SimpleGetEncoding(array('s' => 'Submit')));
  }
...
class TestOfPageScraping extends UnitTestCase {
  ...
  function testLinksPage() {
    ...
    $page = $this->parse($response);
    $this->assertIdentical(
      $page->getUrls(), array('http://www.here.com/a/there.html', 'http://there.com/that.html'));
    $this->assertIdentical(
      $page->getUrlsByLabel('There'), array(new SimpleUrl('http://www.here.com/a/there.html')));
    $this->assertEqual(
      $page->getUrlById('0'), new SimpleUrl('http://there.com/that.html'));
  }
  ...
  function testNastyTitle() {
    ...
    $response->setReturnValue(
      'getContent', '<html><head><Title> <b>Me&amp;Me </TITLE></b></head></html>');
    $page = $this->parse($response);
  ...
  function testEmptyFrameset() {
    $response = new MockSimpleHttpResponse();
    $response->setReturnValue(
      'getContent', '<html><frameset></frameset></html>');
    $page = $this->parse($response);
    $this->assertTrue($page->hasFrames());
    $this->assertIdentical($page->getFrameset(), array());
  }
  ...
  function testSingleFrame() {
    $response = new MockSimpleHttpResponse();
    $response->setReturnValue(
      'getContent', '<html><frameset><frame src="a.html"></frameset></html>');
    $response->setReturnValue('getUrl', new SimpleUrl('http://host/'));
    $page = $this->parse($response);
    $this->assertTrue($page->hasFrames());
    $this->assertIdentical(
      $page->getFrameset(), array(1 => new SimpleUrl('http://host/a.html')));
  }

  function testSingleFrameInNestedFrameset() {
    $response = new MockSimpleHttpResponse();
    $response->setReturnValue('getContent', '<html><frameset><frameset>' .
      '<frame src="a.html">' .
  ...
  function testFrameWithNoSource() {
    $response = new MockSimpleHttpResponse();
    $response->setReturnValue(
      'getContent', '<html><frameset><frame></frameset></html>');
    $page = $this->parse($response);
    $this->assertTrue($page->hasFrames());
    $this->assertIdentical($page->getFrameset(), array());
  }
  ...
  function testFindFormByLabel() {
    $response = new MockSimpleHttpResponse();
    $response->setReturnValue(
      'getContent', '<html><head><form><input type="submit"></form></head></html>');
    $page = $this->parse($response);
    ...
    $this->assertIsA(
      $page->getFormBySubmit(new SimpleByLabel('Submit')), 'SimpleForm');
  }

  function testConfirmSubmitAttributesAreCaseSensitive() {
    $response = new MockSimpleHttpResponse();
    $response->setReturnValue(
      'getContent', '<html><head><FORM><INPUT TYPE="SUBMIT" NAME="S" VALUE="S"></FORM></head></html>');
    $page = $this->parse($response);
    $this->assertIsA(
      $page->getFormBySubmit(new SimpleByName('S')), 'SimpleForm');
    $this->assertIsA(
      $page->getFormBySubmit(new SimpleByLabel('S')), 'SimpleForm');
  }
  ...
  function testFindFormByImage() {
    ...
    $page = $this->parse($response);
    $this->assertIsA(
      $page->getFormByImage(new SimpleByLabel('Label')), 'SimpleForm');
    $this->assertIsA(
      $page->getFormByImage(new SimpleByName('me')), 'SimpleForm');
    $this->assertIsA(
      $page->getFormByImage(new SimpleById(100)), 'SimpleForm');
  }
  ...
  function testFindFormByButtonTag() {
    ...
    $this->assertNull($page->getFormBySubmit(new SimpleByLabel('B')));
    $this->assertIsA(
      $page->getFormBySubmit(new SimpleByName('b')), 'SimpleForm');
    $this->assertIsA(
      $page->getFormBySubmit(new SimpleByLabel('BBB')), 'SimpleForm');
  }

  function testFindFormById() {
    $response = new MockSimpleHttpResponse();
    $response->setReturnValue(
      'getContent', '<html><head><form id="55"><input type="submit"></form></head></html>');
    $page = $this->parse($response);
