...
class SeleniumTestCase extends UnitTestCase {
  ...
  public function setUp() {
    ...
    if (is_null($this->selenium)) {
      $this->selenium = new SimpleSeleniumRemoteControl(
        $this->browser, $this->browserUrl, $this->host, $this->port, $this->timeout
      );
      $this->selenium->start();
    }
  }
    ...
    if (substr($method, 0, 6) == 'verify') {
      return $this->assertTrue(
          call_user_func_array(
            array($this->selenium, $method), $arguments
  ...
  public function verifyText($text) {
    return $this->assertTrue(
        $this->selenium->verifyText($text), sprintf(
          'verifyText failed when on [%s]', $text
        )
    );
  }
  ...
  public function verifyTextPresent($text) {
    return $this->assertTrue(
        $this->selenium->verifyTextPresent($text), sprintf(
          'verifyTextPresent failed when on [%s]', $text
        )
    );
  }
  ...
  public function verifyTextNotPresent($text) {
    return $this->assertTrue(
        $this->selenium->verifyTextNotPresent($text), sprintf(
          'verifyTextNotPresent failed on [%s]', $text
        )
    );
  }
  ...
  public function verifyValue($selector, $value) {
    return $this->assertTrue(
        $this->selenium->verifyValue($selector, $value), sprintf(
          'verifyValue failed on [%s] == [%s]', $selector, $value
        )
    );
  }
  ...
  public function verifyTitle($pattern) {
    return $this->assertTrue(
        $this->selenium->verifyTitle($pattern), sprintf(
          'verifyTitle failed on [%s]', $pattern
        )
    );
  }
