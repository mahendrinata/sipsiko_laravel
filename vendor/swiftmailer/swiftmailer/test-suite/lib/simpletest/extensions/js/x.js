...
function xParent(e, bNode) {
    ...
    if (!bNode && xDef(e.offsetParent))
        ...
        p = e.parentElement;
    else if (xDef(e.parentLayer)) {
        if (e.parentLayer != window)
            p = e.parentLayer;
    }
    return p;
}
...
function xClip(e, iTop, iRight, iBottom, iLeft) {
    ...
    if (e.style) {
        ...
        if (xNum(iLeft)) {
            e.clip.top = iTop;
            e.clip.right = iRight;
            e.clip.bottom = iBottom;
            e.clip.left = iLeft;
        }
        else {
            e.clip.top = 0;
            e.clip.right = xWidth(e);
            e.clip.bottom = xHeight(e);
            e.clip.left = 0;
        }
    }
}
...
function xRemoveEventListener(e, eventType, eventListener, useCapture) {
    ...
    if (e.removeEventListener)
        ...
        e.detachEvent('on' + eventType, eventListener);
    else if (e.releaseEvents) {
        if (useCapture || (eventType.indexOf('mousemove') != -1)) {
            e.releaseEvents(eval('Event.' + eventType.toUpperCase()));
        }
        eval(eh);
    }
    else
...
function xEvent(evt) { // cross-browser event object prototype
    ...
    if (e.target)
        ...
        this.target = xLayerFromPoint(e.pageX, e.pageY);
    if (xOp5or6) {
