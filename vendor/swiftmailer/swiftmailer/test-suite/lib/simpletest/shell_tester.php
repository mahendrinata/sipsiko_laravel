...
class ShellTestCase extends SimpleTestCase {
  ...
  function assertOutput($expected, $message = "%s") {
    $shell = $this->getShell();
    return $this->assert(
        new EqualExpectation($expected), $shell->getOutput(), $message);
  }
  ...
  function assertOutputPattern($pattern, $message = "%s") {
    $shell = $this->getShell();
    return $this->assert(
        new PatternExpectation($pattern), $shell->getOutput(), $message);
  }
  ...
  function assertNoOutputPattern($pattern, $message = "%s") {
    $shell = $this->getShell();
    return $this->assert(
        new NoPatternExpectation($pattern), $shell->getOutput(), $message);
  }
  ...
  function assertFilePattern($pattern, $path, $message = "%s") {
    return $this->assert(
        new PatternExpectation($pattern), implode('', file($path)), $message);
  }
  ...
  function assertNoFilePattern($pattern, $path, $message = "%s") {
    return $this->assert(
        new NoPatternExpectation($pattern), implode('', file($path)), $message);
  }
